        ...
        <div class="col-md-12 grid-margin stretch-card">
                <div class="card bg-inner-page">
                  <div class="card-body">
                    ...
                    <form class="forms-sample" action="{{ route('bill_post_dmt_transaction_retailer') }}" id="dmt_form" method="post">
                        @csrf
                          ...
                          <div class="col-md-9 col-sm-12">
                           ...
                           <select class="js-example-basic-single w-100 form-control" id="beneficiaries" name="beneficiaries" onchange="letsPay()" required="">
                               ...
                               @foreach($data as $r)
                                ...
                                <?php $mobileno = $r->mobileNumber;  ?>
                                @else
                                <?php $mobileno = '';  ?>
                                @endif
                                
                                @if (is_string($r->recipientName))
                                    <option value="{{ $r->recipientId }}">{{ $r->recipientName }} | {{ $r->bankAccountNumber }} | {{ $r->ifsc }} | {{ $mobileno }}</option>
                                @else
                                    <option value="{{ $r->recipientId }}">  | {{ $r->bankAccountNumber }} | {{ $r->ifsc }} | {{ $mobileno }}</option>
                                @endif
                               @endforeach
                           </select>
...
@include('new_pages.retailer.services.bill_dmt.add_benf')
@endsection
