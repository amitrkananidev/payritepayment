        ...
        <div class="col-md-12 grid-margin stretch-card">
                <div class="card bg-inner-page">
                  <div class="card-body">
                    ...
                    <form class="forms-sample" action="{{ route('ace_post_dmt_transaction_retailer') }}" id="dmt_form" method="post">
                        @csrf
                          ...
                          <div class="col-md-9 col-sm-12">
                           ...
                           <select class="js-example-basic-single w-100 form-control" id="beneficiaries" name="beneficiaries" onchange="letsPay()" required="">
                               ...
                               @foreach($data as $r)
                                ...
                                @if (is_string($r->fname))
                                    <option value="{{ $r->id }}#{{ $r->fname }}#{{ $r->AccountNumber }}#{{ $r->ifsc }}#{{ $r->bankname }}">{{ $r->fname }} | {{ $r->AccountNumber }} | {{ $r->ifsc }} | {{ $mobileno }} </option>
                                @else
                                    <option value="{{ $r->id }}#{{ $r->fname }}#{{ $r->AccountNumber }}#{{ $r->ifsc }}#{{ $r->bankname }}">  | {{ $r->AccountNumber }} | {{ $r->ifsc }} | {{ $mobileno }} </option>
                                @endif
                               @endforeach
                           </select>
...
@include('new_pages.retailer.services.ace_dmt.add_benf')
@endsection
