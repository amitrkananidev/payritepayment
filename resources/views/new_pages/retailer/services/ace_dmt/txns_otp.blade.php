                ...
                <div class="card bg-inner-page">
                  <div class="card-body">
                    ...
                    <div class="form-group row">
                        <div class="col-sm-4">
                            <div class="form-check" >
                                <a href="{{ route('ace_dmt_login_retailer') }}" class="button btn btn-default" style="width: 100%;"><span>DMT LOGIN</span></a>
                            </div>
                        </div>
    ...
    function SendOTP(transaction_id){
        ...
        if(transaction_id != ""){
            $.ajax({
                type: 'post',
                dataType:'json',
                url: "{{ route('ace_post_dmt_transaction_otp_send_retailer') }}",
    ...
    function verifyOTP(transaction_id){
        ...
        if(otp != ""){
            $.ajax({
                type: 'post',
                dataType:'json',
                url: "{{ route('ace_post_dmt_transaction_otp_retailer') }}",
                data: {"otp" : otp,"transaction_id" : transaction_id,"latitude":latitude,"longitude":longitude ,"_token":"{{ csrf_token() }}"},
                success: function (result) {
                            loaderHide();
                    if(result.success){
