...
class ApiController extends Controller
{
    ...
    public function balanceReset(Request $request)
    {
        $get = Transaction::where('id','>=','1134375')->where('payable_id','205')->orderBy('id','ASC')->get();
        $last = 752270;
        foreach($get as $r){
            $last_t = $last + $r->amount;
            $diff = $r->balance - $last_t;
            
            // $up = Transaction::find($r->id);
            // $up->balance = $last_t;
            // $up->save();
            
            echo $r->amount."#".$r->balance."#".$last_t."#".$diff."<br>";
            $last = $last_t;
    ...
    {
        exit;
        $transactions = transactions_dmt::select('id','created_at','transaction_id','amount','user_id')->where('event', 'LIKE', 'DMT')
                        ->where('status', 1)
                        ->whereBetween('created_at', ['2025-11-01 00:00:00', '2025-12-06 23:59:59'])
                        ->where('api_id', 0)
                        ->where('comm_settl', 0)
                        ->orderBy('id', 'ASC')
                        ->get();
    // print_r($transactions);
        foreach($transactions as $r){
            ...
            //Commission Start
                        $json_com = $this->WalletCalculation->retailorDmt($txnid);
                        $commission = $json_com['comm'];
                        $tds = $json_com['tds'];
                        $txnid_com = $this->WalletCalculation->txnId('COMM');
                        
                        $user = User::find($r->user_id);
                        $wallets_uuid = $this->WalletCalculation->walletDepositFloat($user->id,$commission,"Commission",'Credit_Retailer_'.$user->mobile.'_DMT_Remittance_Commission',$txnid_com);
                        
                        $ins_com = new user_commissions();
                        $ins_com->user_id = $user->id;
                        $ins_com->transaction_id = $txnid_com;
                        $ins_com->total_amount = $commission + $tds;
                        $ins_com->amount = $commission;
                        $ins_com->tds = $tds;
                        $ins_com->tds_par = 0;
                        $ins_com->wallets_uuid = $wallets_uuid;
                        $ins_com->ref_transaction_id = $txnid;
                        $ins_com->save();
                        ...
                        $update->save();
                    //Commission end
            echo $r->created_at."#".$r->transaction_id.'#'.$r->amount."#".$commission."<br>";
        }
        exit;
        $get = Transaction::where('id','>=','993818')->where('payable_id','351')->orderBy('id','ASC')->get();
        foreach($get as $r){
            echo $r->balance."<br>";
        }
        // $from = $this->UserAuth->fetchFromDate('2025-10-11');
        // $to = $this->UserAuth->fetchToDate('2025-11-05');
        // $gettxns = transactions_dmt::whereBetween('transactions_dmts.created_at', [$from, $to])->where('event','DMT')->where('api_id',0)->where('comm_settl',0)->where('status',1)->get();
        // foreach($gettxns as $r){
        //     $txnid = $r->transaction_id;
        // //Commission Start
        //                 $json_com = $this->WalletCalculation->retailorDmt($txnid);
        //                 $commission = $json_com['comm'];
        //                 $tds = $json_com['tds'];
        //                 $txnid_com = $this->WalletCalculation->txnId('COMM');
        //                 echo $commission."#".$r->transaction_id."#".$r->amount;echo "<br>";
        //                 $user = User::find($r->user_id);
        //                 if($commission > 0){
        //                 $wallets_uuid = $this->WalletCalculation->walletDepositFloat($user->id,$commission,"Commission",'Credit_Retailer_'.$user->mobile.'_DMT_Remittance_Commission',$txnid_com);
    ...
    public function createCustomer(Request $request){
        ...
        if($user){
            ...
            $check_customer = dmt_customers::where('mobile',$request->mobile)->where('status','1')->whereNull('dmt_customers.dmt_type')->first();
            if(!$check_customer){
                $otp = rand(100000, 999999);
                $ins = dmt_customers::where('mobile',$request->mobile)->where('status','0')->whereNull('dmt_customers.dmt_type')->first();
                if($ins){
                    $ins->user_id = $user->id;
    ...
    public function otpCustomerVerify(Request $request){
        ...
        $check_customer = dmt_customers::where('user_id',$user->id)->where('mobile',$request->mobile)->where('otp',$request->otp)->where('status','0')->whereNull('dmt_customers.dmt_type')->first();
        if($check_customer){
            $check_customer->status = 1;
            $check_customer->save();
            $customer = dmt_customers::where('user_id',$user->id)->where('mobile',$request->mobile)->whereNull('dmt_customers.dmt_type')->first();
    ...
    public function doTransactions(Request $request){
        ...
        $validator = Validator::make($request->all(), [
            ...
            "amount" => [
                            ...
                            function($attribute, $value, $fail) {
                                ...
                                if ($value < 10000) {
                                    $fail($attribute . ' must be greater than 10000.');
                                }
                                
                                if ($value > 49999) {
                                    $fail($attribute . ' must be less than 49999.');
                                }
                            },
                        ],
        ...
        if($check_token){
            ...
            return response()->json(['success' => false, 'message' => 'User Not Allow.']);
        }
        
        $allow = [9099153430,8849115650,9016947721,7990234367,9898175934,8128977250,9574152223,8141615959,8141685768,7575860201,9913804277,9898789944,7878277178,9687169137,8980481129,9974350549,9714081414,9173161089,9662082303,9586986215,8238298988,9574692176,9879552424,8153924659,8128006018,9898433923,9825086655,8140988389,9974970630,9033036335,9638122002,6355295950,7990177372,9574075550,8866615678,7405098756,8000777895,9898983219,8799213573,9712719576,9574152223,9853078150,9409433535,9898061583,7862006652,8758750426,9726193583,9998864189,9662154956,7016344125,9067054956,9664587089,7405900721,6260731959,9586000935,9601969196,9879664312,8877222114,8780847447,9724412655,9825488850,9825615077,9913083888,9879399884];
        if (in_array($request->user_mobile, $allow)){
            
        }else{
            return response()->json(['success' => false, 'message' => 'Bank Server Not Working.']);
        }
        ...
        if($user){
            ...
            $get_customer = dmt_customers::where('mobile',$request->customer_mobile)->first();
            ...
            //         }';
            $api_params = '{
                    "AuthCode": "6a31c90d-6d53-482c-9f9a-0ba416e8c514",
                    "Mobile": "'.$request->customer_mobile.'",
                    "BankName": "'.$get_benf->bank_name.'",
                    "AccountNo": "'.$get_benf->account_number.'",
                    "IFSC": "'.$get_benf->ifsc.'",
                    "SenderName": "'.$get_customer->first_name.'",
                    "IsVerify": false,
                    "APIUserRequestID": "'.$txnid.'",
                    "PaymentType": "'.$transfer_type.'",
                    "Amount": '.$amount.',
                    "Name": "'.$get_benf->account_holder_name.'"
                }';
            
            $result = $this->ApiCalls->ipaydigitalPost($api_params);
            ...
            if(isset($json->status)){
                if($json->status == "SUCCESS"){
                    ...
                    $update->status = 1;
                    $update->response_reason = $json->transactionID;
                    $update->utr = $json->transactionID;
                    $update->save();
    ...
    public function callbackIPayDigital(Request $request) {
        $payload = $request->all();
        Log::channel('plutos')->info("CALLBACK");
        Log::channel('plutos')->info($payload);
        
        $status = $payload['Status'];
        
        if($status == "SUCCESS") {
            $txn_id = $payload['RequestID'];
            $utr = $payload['ReferenceNo'];
